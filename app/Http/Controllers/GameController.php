...
class GameController extends Controller
{
    ...
    {
        return view('index', ['games' => Game::orderBy('updated_at', 'desc')->take(15)->get(), 'games_sub' => Game::getGamesBySubs()->take(15)->get(), 'popular' => Game::getPopular5Games(), 'top_authors' => User::getTopAuthors(), 'tags' => Tag::orderBy('name', 'desc')->get(), 'notifications'=>auth()->check()? Notification::where('user_id', auth()->id())->get():null]);
    }

    public function catalog($id = null)
    {
        return view('games.catalog', ['games' => $id?Game::whereIn('id',GameTag::where('tag_id', $id)->get()->pluck('game_id') )->orderBy('updated_at', 'desc')->get():Game::orderBy('updated_at', 'desc')->get(), 'tags' => Tag::get(), 'filter_tag'=>$id? Tag::find($id)->name : null]);
    }
    ...
    public function filter(Request $request)
    {
        ...
        return back()->withInput(['filter_games' => $games->get()]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $res = Game::create($request->except('screens', 'cover', '_token', 'tags', 'files') + ['cover' => $path, 'count_downloads' => 0, 'count_likes' => 0, 'count_dislikes' => 0, 'baned'=> false, 'user_id' => auth()->id(), 'access_id' => 1]);
        if ($request->screens) {
    ...
    {
        return GameImage::find($request->data)->delete();
    }
